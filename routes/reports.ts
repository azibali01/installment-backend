...
router.get("/dashboard", authenticate, authorizePermission("view_reports"), async (req: Request, res: Response) => {
  try {
    ...
    const baseUnwind = [
      ...
      { $addFields: { "installmentSchedule.remaining": { $subtract: ["$installmentSchedule.amount", { $ifNull: ["$installmentSchedule.paidAmount", 0] }] } } },
      { $match: { "installmentSchedule.remaining": { $gt: 0 } } },
    ]

    const makeCountPipeline = (dateMatch: any) => [
      ...baseUnwind,
      { $match: dateMatch },
      { $group: { _id: null, count: { $sum: 1 }, totalRemaining: { $sum: "$installmentSchedule.remaining" } } },
    ]

    const makeListPipeline = (dateMatch: any, limit = 10) => [
      ...baseUnwind,
      { $match: dateMatch },
    ...
    const [
      ...
      InstallmentPlan.aggregate(makeCountPipeline({ "installmentSchedule.dueDate": { $gte: startOfDay, $lte: endOfDay } })),
      InstallmentPlan.aggregate(makeCountPipeline({ "installmentSchedule.dueDate": { $gt: endOfDay, $lte: upcomingEnd } })),
      InstallmentPlan.aggregate(makeCountPipeline({ "installmentSchedule.dueDate": { $lt: startOfDay } })),
      InstallmentPlan.aggregate(makeListPipeline({ "installmentSchedule.dueDate": { $gte: startOfDay, $lte: endOfDay } }, 10)),
      InstallmentPlan.aggregate(makeListPipeline({ "installmentSchedule.dueDate": { $gt: endOfDay, $lte: upcomingEnd } }, 10)),
      InstallmentPlan.aggregate(makeListPipeline({ "installmentSchedule.dueDate": { $lt: startOfDay } }, 10)),
      Payment.aggregate([{ $group: { _id: null, total: { $sum: "$amount" } } }]),
      Expense.aggregate([{ $group: { _id: null, total: { $sum: "$amount" } } }]),
    ])

    const normalize = (arr: any[]) => (arr[0] ? { count: arr[0].count, totalRemaining: arr[0].totalRemaining } : { count: 0, totalRemaining: 0 })
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : "Failed to generate dashboard" })
  }
})
